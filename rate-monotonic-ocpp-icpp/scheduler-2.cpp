...
BaseType_t xWorkIsDone; /* pdFALSE if the job is not finished, pdTRUE if the
job is finished. */
...
schedUSE_TIMING_ERROR_DETECTION_DEADLINE == 1)
...
#if (schedSCHEDULING_POLICY == schedSCHEDULING_POLICY_RMS || schedSCHEDULING_POLICY
== schedSCHEDULING_POLICY_DMS)
...
typedef struct taskResourceObject {
int priorityCeiling;
...
/* Array for task resources. */ //CCCC
static taskResourceObject resourceArray[NUM_RESOURCES] = { 0 };
void prvInitResourceArray(void)
{
UBaseType_t uxIndex;
for (uxIndex = 0; uxIndex < NUM_RESOURCES; uxIndex++)
{
resourceArray[uxIndex].priorityCeiling = 1;
resourceArray[uxIndex].resource = xSemaphoreCreateMutex();
resourceArray[uxIndex].lockedState = false;
//Serial.print("RESOURCE CREATED index: ");
//Serial.println(uxIndex);
}
vSetResourceCeiling(0, 5);
 vSetResourceCeiling(1, 4);
}
//static void vSetResourceCeiling(int index, TaskHandle_t xTaskHandle){
// resourceArray[uxIndex].priorityCeiling = xTaskHandle->;
//}
void vSetResourceCeiling(int uxIndex, int priority){
Serial.println(uxIndex);
Serial.println(priority);
Serial.println(resourceArray[uxIndex].priorityCeiling);
if (priority > resourceArray[uxIndex].priorityCeiling){
resourceArray[uxIndex].priorityCeiling = priority;
//Serial.print ("Resource Changed to!!");
//Serial.println (resourceArray[uxIndex].priorityCeiling);
}
}
BaseType_t highestPriorLocked(){
BaseType_t highestPrior = 0;
UBaseType_t uxIndex;
for (uxIndex = 0; uxIndex < NUM_RESOURCES; uxIndex++)
{
if(highestPrior < resourceArray[uxIndex].priorityCeiling &&
resourceArray[uxIndex].lockedState)
highestPrior = resourceArray[uxIndex].priorityCeiling;
//Serial.print("RESOURCE CREATED index: ");
//Serial.println(uxIndex);
}
return highestPrior;
}
void vSchedulerResourceWait(int resourceIndex){
//BaseType_t oldPrior = pxThisTask->uxPriority;
SchedTCB_t* pxThisTask;
TaskHandle_t xCurrentTaskHandle = xTaskGetCurrentTaskHandle();
BaseType_t xIndex = prvGetTCBIndexFromHandle(xCurrentTaskHandle);
pxThisTask = &xTCBArray[xIndex];
#if (resourceSCHEDULING_POLICY == resourceSCHEDULING_POLICY_OCPP)
BaseType_t highestLockedCeiling = highestPriorLocked();
while (highestLockedCeiling > pxThisTask->uxPriority){
}
#endif
if(!xSemaphoreTake( resourceArray[resourceIndex].resource, ( TickType_t )
10000 ) == pdTRUE )
{
//Serial.println("SEMA TAKEN!");
#if (resourceSCHEDULING_POLICY == resourceSCHEDULING_POLICY_ICPP)
resourceArray[resourceIndex].oldPrior = pxThisTask->uxPriority;
pxThisTask->uxPriority =
resourceArray[resourceIndex].priorityCeiling;
#endif
resourceArray[resourceIndex].lockedState = true;
//Serial.print("Set pripiryt up to");
//Serial.println(resourceArray[resourceIndex].priorityCeiling);
}
else{
//Serial.println("SEMA TAKE FAIL!__________________");
}
//Serial.println("NEW PRIOR");
//Serial.println(pxThisTask->uxPriority);
}
void vSchedulerResourceSignal(int resourceIndex){
SchedTCB_t* pxThisTask;
TaskHandle_t xCurrentTaskHandle = xTaskGetCurrentTaskHandle();
BaseType_t xIndex = prvGetTCBIndexFromHandle(xCurrentTaskHandle);
if( xSemaphoreGive( resourceArray[resourceIndex].resource))
{
//Serial.println("SEMA GIVEN!");
//Serial.println("GIVE DROP FROM");
resourceArray[resourceIndex].lockedState = false;
//Serial.println(pxThisTask->uxPriority);
#if (resourceSCHEDULING_POLICY == resourceSCHEDULING_POLICY_ICPP)
pxThisTask = &xTCBArray[xIndex];
pxThisTask->uxPriority = resourceArray[resourceIndex].oldPrior;
#endif
}
else{
//Serial.println("SEMA GIVE FAIL!______________");
}
}
...
TaskHandle_t* pxCreatedTask, TickType_t xPhaseTick, TickType_t xPeriodTick,
...
BaseType_t xReturnValue = xTaskCreate(prvPeriodicTaskCode, pxTCB-
>pcName, pxTCB->uxStackDepth,
...
/* your implementation goes here */
if (pxTCB != NULL && pxTCB->xWorkIsDone == pdFALSE && pxTCB->xExecutedOnce ==
pdTRUE)
{
pxTCB->xAbsoluteDeadline = pxTCB->xLastWakeTime + pxTCB-
>xRelativeDeadline;
...
{
pxTCB = &xTCBArray[xIndex];
if ((pxTCB) && (pxTCB->xInUse == pdTRUE) && (pxTCB-
>pxTaskHandle != NULL))
{
prvSchedulerCheckTimingError(xTickCount, pxTCB);
Serial.print("CURENT PRIOR");
//Serial.println(uxTaskPriorityGet(xTCBArray[xIndex]));
}
}
#endif
#endif /* schedUSE_TIMING_ERROR_DETECTION_DEADLINE ||

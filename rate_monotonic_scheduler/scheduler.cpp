...
static void prvCreateAllTasks( void );
	...
	static BaseType_t prvFindEmptyElementIndexTCB( void )
	{
		...
		//CCC
    BaseType_t returnVal = -1;
		for(BaseType_t i =0; i < schedMAX_NUMBER_OF_PERIODIC_TASKS; i++){
      if (xTCBArray[i].xInUse == pdFALSE) return i;
    }
    return returnVal;
	}
	...
	{
		//CC??
    bool useflag = xTCBArray[xIndex == 1].xInUse;
    if (useflag) {
      xTaskCounter--;
...
void vSchedulerPeriodicTaskDelete( TaskHandle_t xTaskHandle )
{
    ...
    BaseType_t handlePrev = prvGetTCBIndexFromHandle(xTaskHandle);
		prvDeleteTCBFromArray(handlePrev);
...
static void prvSetFixedPriorities( void )
{
    ...
    ////CCCC
		...
		///CCC
    TickType_t newDeadline = pxTCB->xReleaseTime + pxTCB->xRelativeDeadline;
    pxTCB->xAbsoluteDeadline = newDeadline;
    ...
    if (pxTCB->xWorkIsDone == pdFALSE && pxTCB->xExecutedOnce == pdTRUE){
      TickType_t newDeadline = pxTCB->xAbsoluteDeadline -xTickCount;
      if (newDeadline < 0) {
	...
	static void prvSchedulerCheckTimingError( TickType_t xTickCount, SchedTCB_t *pxTCB )
	{
		///CCCCC
    #if( schedUSE_TCB_ARRAY == 1 )
			if( pdFALSE == pxTCB->xInUse ) return;
		#endif

		#if( schedUSE_TIMING_ERROR_DETECTION_DEADLINE == 1 )						
			/* check if task missed deadline */
            //CCC
            TickType_t timeDiff = xTickCount - pxTCB->xLastWakeTime;
            if (timeDiff > 0){
	...
	static void prvSchedulerFunction( void *pvParameters )
	{
        ...
        #if (sched_TCB_ARRAY)
          for (BaseType_t i = 0; i < schedMAX_NUMBER_OF_PERIODIC_TASKS; i++){
            prvSchedulerCheckTimingError(xTickCount, &xTCBArray[i]);
          }
        #endif
